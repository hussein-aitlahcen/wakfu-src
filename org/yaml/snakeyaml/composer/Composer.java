...
public class Composer
{
    ...
    private Node composeNode(final Node parent) {
        ...
        if (!this.parser.checkEvent(Event.ID.Alias)) {
            final NodeEvent event = (NodeEvent)this.parser.peekEvent();
            String anchor = null;
        ...
        final AliasEvent event2 = (AliasEvent)this.parser.getEvent();
        String anchor = event2.getAnchor();
        if (!this.anchors.containsKey(anchor)) {
            throw new ComposerException(null, null, "found undefined alias " + anchor, event2.getStartMark());
        }
    ...
    private Node composeMappingNode(final String anchor) {
        ...
        while (!this.parser.checkEvent(Event.ID.MappingEnd)) {
            final Node itemKey = this.composeNode(node);
            ...
            final Node itemValue = this.composeNode(node);
