...
class Floor1 extends FuncFloor<InfoFloor1, LookFloor1>
{
    private static final int VIF_POSIT = 63;
    private static final float[] FLOOR_fromdB_LOOKUP;
    
    @Override
    InfoFloor1 unpack(final Info vi, final Buffer opb) {
        int count = 0;
        int maxclass = -1;
    ...
    @Override
    LookFloor1 look(final DspState vd, final InfoMode mi, final InfoFloor1 info) {
        int _n = 0;
        ...
        for (int i = 0; i < _n - 1; ++i) {
            for (int k = i; k < _n; ++k) {
                if (info.postlist[sortpointer[i]] > info.postlist[sortpointer[k]]) {
                    final int foo = sortpointer[k];
                    sortpointer[k] = sortpointer[i];
                    sortpointer[i] = foo;
                }
            }
        }
        ...
        for (int i = 0; i < _n - 2; ++i) {
            ...
            int lx = 0;
            int hx = look.n;
            final int currentx = info.postlist[i + 2];
            for (int l = 0; l < i + 2; ++l) {
                final int x = info.postlist[l];
                if (x > lx && x < currentx) {
                    lo = l;
                    lx = x;
                }
    ...
    @Override
    int[] inverse1(final Block vb, final LookFloor1 look, final Object memo) {
    ...
    @Override
    int inverse2(final Block vb, final LookFloor1 look, final Object memo, final float[] out) {
        final InfoFloor1 info = look.vi;
    ...
    {
        static final int VIF_POSIT = 63;
        final int[] sorted_index;
        final int[] forward_index;
        final int[] reverse_index;
        final int[] hineighbor;
