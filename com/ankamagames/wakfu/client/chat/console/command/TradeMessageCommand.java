...
public class TradeMessageCommand implements Command
{
    @Override
    public void execute(final ConsoleManager manager, final CommandPattern pattern, final ArrayList<String> args) {
        String commandLine = args.get(0).trim();
        final String command = ChatConfigurator.getCommandForChannelName(ChatCommandsParameters.TRADE.getCommandName()).getCommand();
        ...
        if (localPlayer != null && !commandLine.isEmpty()) {
            ...
            if (commandLine != null) {
                ...
                final UserTradeContentMessage tradeMessage = new UserTradeContentMessage();
                tradeMessage.setMessageContent(commandLine);
                WakfuGameEntity.getInstance().getNetworkEntity().sendMessage(tradeMessage);
