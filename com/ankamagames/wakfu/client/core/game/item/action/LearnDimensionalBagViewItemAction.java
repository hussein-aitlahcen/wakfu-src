...
public class LearnDimensionalBagViewItemAction extends AbstractClientItemAction
{
    private static final Logger m_logger;
    
    public LearnDimensionalBagViewItemAction(final int id) {
        super(id);
    }
    ...
    public boolean run(final Item item) {
        ...
        if (character.getBags().getItemFromInventories(item.getUniqueId()) == null) {
            LearnDimensionalBagViewItemAction.m_logger.error((Object)"[ItemAction] On essaye de lancer une action avec un item qui n'est pas dans les bags");
            return false;
        }
    ...
    public ItemActionConstants getType() {
        return ItemActionConstants.LEARN_DIMENSIONAL_BAG_VIEW;
    }
    
    static {
        m_logger = Logger.getLogger((Class)LearnDimensionalBagViewItemAction.class);
    }
}
