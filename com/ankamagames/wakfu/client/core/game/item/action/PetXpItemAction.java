...
public class PetXpItemAction extends AbstractClientItemAction
{
    private static final Logger m_logger;
    private int m_numLevels;
    
    public PetXpItemAction(final int id) {
        super(id);
    }
    
    @Override
    public void parseParameters(final String[] params) {
        this.m_numLevels = Integer.valueOf(params[0]);
    }
    ...
    public boolean run(final Item item) {
        ...
        if (character.getBags().getItemFromInventories(item.getUniqueId()) == null) {
            PetXpItemAction.m_logger.error((Object)"[ItemAction] On essaye de lancer une action avec un item qui n'est pas dans les bags");
            return false;
        }
    ...
    public ItemActionConstants getType() {
        return ItemActionConstants.PET_XP;
    }
    
    static {
        m_logger = Logger.getLogger((Class)PetXpItemAction.class);
    }
}
