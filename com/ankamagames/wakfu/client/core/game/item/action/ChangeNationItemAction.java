...
public class ChangeNationItemAction extends AbstractClientItemAction
{
    private static final Logger m_logger;
    private int m_nationId;
    
    public ChangeNationItemAction(final int id) {
        super(id);
    }
    ...
    public boolean run(final Item item) {
        ...
        if (character.getBags().getItemFromInventories(item.getUniqueId()) == null) {
            ChangeNationItemAction.m_logger.error((Object)"[ItemAction] On essaye de lancer une action avec un item qui n'est pas dans les bags");
            return false;
        }
    ...
    public void parseParameters(final String[] params) {
        this.m_nationId = Integer.parseInt(params[0]);
    }
    ...
    public ItemActionConstants getType() {
        return ItemActionConstants.CHANGE_NATION;
    }
    
    static {
        m_logger = Logger.getLogger((Class)ChangeNationItemAction.class);
    }
}
