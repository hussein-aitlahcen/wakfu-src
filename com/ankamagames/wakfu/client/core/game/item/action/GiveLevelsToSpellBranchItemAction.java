...
public class GiveLevelsToSpellBranchItemAction extends AbstractClientItemAction
{
    private static final Logger m_logger;
    
    public GiveLevelsToSpellBranchItemAction(final int id) {
        super(id);
    }
    ...
    public boolean run(final Item item) {
        ...
        if (character.getBags().getItemFromInventories(item.getUniqueId()) == null) {
            GiveLevelsToSpellBranchItemAction.m_logger.error((Object)"[ItemAction] On essaye de lancer une action avec un item qui n'est pas dans les bags");
            return false;
        }
    ...
    public ItemActionConstants getType() {
        return ItemActionConstants.GIVE_LEVELS_TO_SPELL_BRANCH;
    }
    
    static {
        m_logger = Logger.getLogger((Class)GiveLevelsToSpellBranchItemAction.class);
    }
}
