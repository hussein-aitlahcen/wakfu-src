...
public class AddKamasItemAction extends AbstractClientItemAction
{
    private static final Logger m_logger;
    
    public AddKamasItemAction(final int id) {
        super(id);
    }
    ...
    public boolean run(final Item item) {
        ...
        if (character.getBags().getItemFromInventories(item.getUniqueId()) == null) {
            AddKamasItemAction.m_logger.error((Object)"[ItemAction] On essaye de lancer une action avec un item qui n'est pas dans les bags");
            return false;
        }
    ...
    public ItemActionConstants getType() {
        return ItemActionConstants.GIVE_KAMAS;
    }
    
    static {
        m_logger = Logger.getLogger((Class)AddKamasItemAction.class);
    }
}
